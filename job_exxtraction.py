...
OUTPUT_JSON = os.getenv("OUTPUT_JSON", "linkedin_jobs.json")
OUTPUT_CSV = os.getenv("OUTPUT_CSV", "linkedin_jobs.csv")
DEBUG_HTML_FILE = os.getenv("DEBUG_HTML_FILE", "debug_page.html")
DETAIL_DEBUG_HTML_FILE = os.getenv("DETAIL_DEBUG_HTML_FILE", "debug_job_detail.html")
...
def login(page):
    ...
    try:
        ...
        print("Security check or alternate login flow detected. Complete it manually within 60 seconds...")
...
def title_matches_filter(job_title, required_titles):
    """Return True if job_title exactly matches any entry in required_titles."""
    if not required_titles:
        return True
    normalized_job = normalize_title(job_title)
    return any(normalized_job == normalize_title(t) for t in required_titles)
    ...
    with sync_playwright() as playwright:
        ...
        for keyword in active_config["keywords_list"]:
            ...
            for location, geo_id in location_pairs:
                ...
                print(f"Titles  : {active_config['job_titles']}")
                ...
                if total:
                    print(f"Total jobs reported by LinkedIn: {total}")
                ...
                for page_num in range(active_config["max_pages"]):
                    ...
                    print(f"\nPage {page_num + 1} (start={start_offset})")
                    ...
                    print(f"   Extracted {len(jobs)} cards from this page.")
                    ...
                    for job in jobs:
                        ...
                        if not title_matches_filter(job.get("title"), active_config["job_titles"]):
                            ...
                            print(
                                f"   Skip (title): {job.get('title')}"
                            )
                            continue

                        seen_urls.add(url)
                        print(
                            f"   [{len(all_jobs) + 1}/{active_config['max_jobs']}] "
                            f"Fetching: {job.get('title')} @ {job.get('company')}"
                        )
                    ...
                    print(
                        f"   +{new_count} matched | skipped {skipped_title_count} by title | "
                        f"total {len(all_jobs)}/{active_config['max_jobs']}"
                    )
                    ...
                    if len(all_jobs) >= active_config["max_jobs"]:
                        print(f"   Reached target of {active_config['max_jobs']} jobs.")
                        break

                    if not jobs:
                        print("   No more results for this keyword+location.")
                        break

                    if page_num > 0 and new_count == 0:
                        print("   No new title matches on this page. Stopping pagination.")
                        break
...
if __name__ == "__main__":
    ...
    collected = scrape_all_jobs(SEARCH_CONFIG)
    print(f"\nTotal exact title-matched jobs collected: {len(collected)}")
